...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.phone || !formData.password) {
      notify('সব ফিল্ড পূরণ করুন', 'error')
      setIsLoading(false)
      return
    }

    if (formData.password !== formData.confirmPassword) {
      notify('পাসওয়ার্ড মিলছে না', 'error')
      setIsLoading(false)
      return
    }

    if (formData.password.length < 6) {
      notify('পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের হতে হবে', 'error')
      setIsLoading(false)
      return
    }
    ...
    if (result.success) {
      notify('সফলভাবে রেজিস্ট্রেশন সম্পন্ন হয়েছে', 'success')
      // Redirect to home after successful registration
      navigate('/', { replace: true })
    } else {
      notify(result.message || 'রেজিস্টারেশন ব্যর্থ হয়েছে', 'error')
      setIsLoading(false)
    }
  }
